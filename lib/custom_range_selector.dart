...
class CustomRangeSelector extends StatefulWidget {
  ...
  const CustomRangeSelector({Key key, this.start, this.end, this.divisions, this.onStartChange, this.onEndChange, this.width, this.height}) : super(key: key);
...
class _CustomRangeSelectorState extends State<CustomRangeSelector> {
  ...
  void initState(){
    ...
    topDividers = markers.map((mp) => Positioned(
      top: 0,
      left: mp -1,      
      child: Container(
        color: Colors.black,
    ...
    bottomDividers = markers.map((mp) => Positioned(
      bottom: 0,
      left: mp - 1,      
      child: Container(
        color: Colors.black,
